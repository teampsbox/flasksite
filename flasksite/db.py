...
#close_db checks if a connect was created by checking if g.db was set. If the connection exists, it is closed. Further down you will tell your application about the close_db function in the application factory so that it is called after each request.
def close_db(e=None):
    db = g.pop('db', None)
...
# Add the Python functions that will run these SQL commands(schema.sql) to the db.py file:
def init_db():
    #get_db returns a database connection, which is used to execute the commands read from the file.
    db = get_db()

    #open_resource() opens a file relative to the flasksite package, which is useful since you won't necessarily know where that location is when deploying the application later.
